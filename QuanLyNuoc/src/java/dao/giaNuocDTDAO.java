...
public class giaNuocDTDAO {
    ...
    public List<giaNuocDT> getAllgiaNuocDT(){
        ...
        String query = "SELECT * FROM [TienNuoc].[dbo].[GiaNuocDoanThe] ORDER BY namApDung";
        try {
    ...
    public giaNuocDT getGiaNuocById(String id){
     ...
     public giaNuocDT getGiaNuocIDMax(){
        giaNuocDT u = new giaNuocDT();
        String query = "SELECT * FROM [TienNuoc].[dbo].[GiaNuocDoanThe] WHERE Id = (SELECT MAX(Id) FROM [TienNuoc].[dbo].[GiaNuocDoanThe])";
        try {
            conn = new ConnectDB().getConnection();//mo ket noi voi sql
            ps = conn.prepareStatement(query);
            rs = ps.executeQuery();
